...
const Filter = () => {
  ...
  const fetchProducts = async (pageNumber) => {
    try {
      const response = await fetch(`http://localhost:3001/product-related/getProducts?page=${pageNumber}&limit=5`);
  ...
  return (
    <div className=" mt-4 px-0" style={{ maxWidth: "100vw" }}>
      { /*className="row "*/}
      <div className="filter-products-yash" >

        <div className={`filter-yash ${isFilterOpen ? "open" : ""}  col-md-2 me-4 mt-1`} >
          <div className="accordion" id="filtersAccordion">
            ...
            {filters.map((filter, index) => (
              <div className="accordion-item rounded-0" key={index}>
                ...
                <div
                  className={`accordion-collapse collapse ${openFilters.includes(index) ? "show" : ""
                    }`}
                >
        ...
        <button className="floating-filter-btn" onClick={() => setIsFilterOpen(!isFilterOpen)}>
          {isFilterOpen ? "Close Filters" : "Filter +"}
        </button>


        <div className="col-md-9 disc-productList " style={{ fontSize: "15px" }}>
